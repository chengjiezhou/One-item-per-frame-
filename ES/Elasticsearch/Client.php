...
class Client
{
    ...
    public function clearCache($params = [])
    {
        ...
        $endpointBuilder = $this->endpoints;
        /** @var \Elasticsearch\Endpoints\Ping $endpoint */
        $endpoint = $endpointBuilder('ClearCache');
        try {
            $response = $endpoint->setParams($params)->performRequest();
            $endpoint->resultOrFuture($response);
        } catch (Missing404Exception $exception) {
    ...
    /**
     * by liqian on 2016-07-15
     * @param $params array Associative array of parameters
     *
     * @return array
     */
    public function refresh($params = []){
        return $this->indices->refresh($params);
    }
    /**
    ...
    /**
     * update 2015-12-15 By Liqian
     * Compared with the function search，the difference is $endpointBuilder('SearchEx')
     * $params['index']                    = (list) A comma-separated list of index names to search; use `_all` or empty string to perform the operation on all indices
     *        ['type']                     = (list) A comma-separated list of document types to search; leave empty to perform the operation on all types
     *        ['analyzer']                 = (string) The analyzer to use for the query string
     *        ['analyze_wildcard']         = (boolean) Specify whether wildcard and prefix queries should be analyzed (default: false)
     ...
     * @return array
     */
    public function searchEx($params = array())
    {
        $index = $this->extractArgument($params, 'index');
        $type = $this->extractArgument($params, 'type');
        $body = $this->extractArgument($params, 'body');
        ...
        /** @var \Elasticsearch\Endpoints\Search $endpoint */
        $endpoint = $endpointBuilder('SearchEx');
        $endpoint->setIndex($index)
            ->setType($type)
            ->setBody($body);
        $endpoint->setParams($params);
